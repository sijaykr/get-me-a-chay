...
const PaymentPage = ({ username }) => {
  ...
  useEffect(() => {
    if (searchParams.get("paymentdone") === "true") {
      toast("Thanks for your donation!", { position: "top-right", autoClose: 5000, transition: Bounce });
  ...
  const pay = async (amtInPaise) => {
    ...
    try {
      ...
      const options = {
        ...
        image: currentUser.profilepic || "/avatar.gif",
        order_id: orderId,
        callback_url: `${process.env.NEXT_PUBLIC_URL || "http://localhost:3000"}/api/razorpay`,
        prefill: { name: paymentform.name, email: "email@example.com", contact: "9999999999" },
        notes: { name: paymentform.name, message: paymentform.message, amount: amtInPaise / 100, to_username: username },
        theme: { color: "#3399cc" },
      };
      ...
      if (sdkLoaded && typeof window !== "undefined" && window.Razorpay) {
        ...
        rzp.open();
      } else toast.error("Payment system not ready. Please try again.");
    } catch (err) {
          ...
          <div className="flex flex-col md:flex-row gap-4 my-10 w-full max-w-4xl">
            <div className="w-full md:w-1/2 bg-slate-900 rounded-lg text-white p-6">
              ...
              <ul className="space-y-2 text-xl text-center">
                {payments.length === 0 ? <li>No payments yet</li> : payments.map((p, idx) => (
                  <li key={idx} className="my-2 flex gap-2 items-center">
                    <Image src="/prop.gif" className="w-16 rounded-full" alt="supporter" width={64} height={64} />
                    <span>{p.name} donated <span className="font-bold">₹{p.amount}</span> with message <q>{p.message}</q></span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="w-full md:w-1/2 bg-slate-900 rounded-lg text-white p-6 text-center">
              ...
              <div className="flex flex-col gap-2">
                <input onChange={handleChange} value={paymentform.name} name="name" type="text" className="w-full text-white font-bold p-3 rounded-lg bg-slate-600" placeholder="Enter Name" />
                <input onChange={handleChange} value={paymentform.message} name="message" type="text" className="w-full text-white font-bold p-3 rounded-lg bg-slate-600" placeholder="Enter Message" />
                <input onChange={handleChange} value={paymentform.amount} name="amount" type="number" className="w-full text-white font-bold p-3 rounded-lg bg-slate-600" placeholder="Enter Amount" />
                <button type="button" className="bg-gradient-to-r from-purple-500 to-pink-500 p-3 rounded-lg hover:cursor-pointer" onClick={() => pay(Number(paymentform.amount) * 100)} disabled={paymentform.name.length < 3 || paymentform.message.length < 4 || paymentform.amount.length < 1}>
